...
func (s *MetricsCollector) GetNVIDIAGPUMetrics(ctx context.Context) ([]schemas.GPUMetrics, error) {
	// NB: MIG-enabled GPUs are not handled here. Under MIG, nvidia-smi cannot
	// report per-instance utilization (and `--query-gpu` returns N/A or
	// "[Insufficient Permissions]"), so MIG GPU metrics are collected entirely
	// out-of-band via the shim's DCGM exporter and merged server-side.
